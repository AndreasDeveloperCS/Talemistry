...
@Controller('position-benefits')
@SetMetadata('entityModel', PositionBenefit)
export class MotivationalFactorsController extends BaseController<PositionBenefit> {
    override className: string = this.constructor.name;

    constructor(protected service: PositionBenefitsService,
    ...
    async post(
        ...
        try {
            console.log(`PositionBenefit Inserted entity:`, body);
            const requestingUser: User = this.utilitiesService?.getUser(request);
            console.log(`PositionBenefit Inserted entity userId:`, body, requestingUser._id);
            const insertedEntity: PositionBenefit = await this.service.createIfNotExists(body, requestingUser._id);
            console.log(`PositionBenefit Inserted entity:`, insertedEntity);
