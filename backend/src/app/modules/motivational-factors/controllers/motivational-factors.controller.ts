...
@Controller('motivational-factors')
@SetMetadata('entityModel', MotivationalFactor)
export class MotivationalFactorsController extends BaseController<MotivationalFactor> {
    override className: string = this.constructor.name;

    constructor(protected service: MotivationalFactorsService,
    ...
    async post(
        ...
        try {
            console.log(`MotivationalFactor Inserted entity:`, body);
            const requestingUser: User = this.utilitiesService?.getUser(request);
            if (requestingUser) {
                body.isVerified = true;
                body.createdBy = requestingUser._id;
            }
            console.log(`MotivationalFactor Inserted entity userId:`, body, requestingUser._id);
            const insertedEntity: MotivationalFactor = await this.service.createIfNotExists(body, requestingUser._id);
            console.log(`MotivationalFactor Inserted entity:`, insertedEntity);

            return response.status(insertedEntity ? 201 : 200).send(insertedEntity);
        } catch (error: any) {
            if (error?.code === 11000) {
                const existing = await this.service['model']
                    .findOne({ factor: body.factor?.trim() })
