...
@Controller('recruitment-platforms')
@SetMetadata('entityModel', RecruitmentPlatform)
export class RecruitmentPlatformController extends BaseController<RecruitmentPlatform> {
  ...
  async getAllAsync(
    ...
    try {
      ...
      // const result = await this.linkedInAdapterService.getAccessToken(this.linkedinCode);
      //  // console.log(result);
    ...
    FileInterceptor('icon', {
      storage: diskStorage({
        destination: (req, file, cb) => {
          const currentDir = getBaseDir('post');
  ...
  async put(
    ...
    try {
      ...
      const absolutePath = path.posix.join(getBaseDir('put'), relativePath);
