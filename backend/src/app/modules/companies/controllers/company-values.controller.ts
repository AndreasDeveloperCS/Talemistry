...
@Controller('company-values')
@SetMetadata('entityModel', CompanyValue)
export class CompanyValuesController extends BaseController<CompanyValue> {
    override className: string = this.constructor.name;

    constructor(protected service: CompanyValuesService,
    ...
    async post(
        ...
        try {
            console.log(`CompanyValue Inserted entity:`, body);
            const requestingUser: User = this.utilitiesService?.getUser(request);
            console.log(`CompanyValue Inserted entity userId:`, body, requestingUser._id);
            const insertedEntity: CompanyValue = await this.service.createIfNotExists(body, requestingUser._id);
            console.log(`CompanyValue Inserted entity:`, insertedEntity);

            return response.status(insertedEntity ? 201 : 200).send(insertedEntity);
        } catch (error: any) {
            if (error?.code === 11000) {
                const existing = await this.service['model']
                    .findOne({ value: body.value?.trim() })
