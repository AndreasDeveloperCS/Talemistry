...
// let recStartTime;
// let recElapsedTime;
// let tabtheme = 'dark'; // dark - grey ...
// let btnBar = 'vertical'; // vertical - horizontal
...
//     themeSelect = getId('tabtheme');
//     videoObjFitSelect = getId('videoObjFitSelect');
//     btnsBarSelect = getId('btnBar');
...
//         openURL(
//             `/c?roomId=${roomId}&getUserMediaError=Not allowed both Audio and Video <br/> Check the common getusermedia errors <a href="https://blog.addpipe.com/common-getusermedia-errors" target="_blank">here<a/>`,
//         );
//     }
// }
...
//         openURL(
//             `/c?roomId=${roomId}&getUserMediaError=${err.toString()} <br/> Check the common getusermedia errors <a href="https://blog.addpipe.com/common-getusermedia-errors" target="_blank">here<a/>`,
//         );
//     }
...
//     chatDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('chat_channel');
...
//     fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('file_sharing_channel');
...
let isChatPasteTxt = false;
let needToCreateOffer = false; // after session description answer
//let signalingSocket = io({ transports: ['websocket'] }); // socket.io connection to our webserver
let signallingSocket;
let localMediaStream; // my microphone / webcam
let remoteMediaStream; // peers microphone / webcam
let recScreenStream; // recorded screen stream
let remoteMediaControls = false; // enable - disable peers video player controls (default false)
...
function getRoomId() {
    ...
    let roomId = queryRoomId ? queryRoomId : location.pathname.substring(ind + 1);
    if (roomId.includes('?'))
...
function initClientPeer() {
    ...
    // Disable the HTTP long-polling transport
    //signalingSocket = io('http://localhost:5005',{ transports: ['websocket'] });
    signalingSocket = io({ transport: ['websocket'] });
    //signalingSocket = io();
...
function welcomeUser() {
    const myRoomUrl = window.location.href;
    console.log('Room name: ', myRoomUrl);
    Swal.fire({
        background: swalBackground,
        position: 'center',
        title: '<strong>Welcome ' + myPeerName + '</strong>',
