...
import { Filtering, FilterRule, PaginatedResource, Sorting } from '../../../general/services/search-logic.service';
import { PositionsService } from '../../../positions/services/positions.service';
import { AuthGuardService } from '../../../authentication/guard/auth-guard.service';
import { OwnerEntity, PositionEntity, VerifiedEntity } from '../../../general/models/base-entity';
...
@Component({
  selector: 'app-saved-positions',
  templateUrl: './saved-positions.component.html',
  styleUrl: './saved-positions.component.scss',
  standalone: false,
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class SavedPositionsComponent extends TableTemplateComponent<OpenPosition> implements OnInit, AfterViewInit, OnDestroy {
  ...
  override getList(selectedPageSize: number, pageIndex: number, sorting: Sorting, filtering: Filtering) {
      this.positionsService.getAllSavedAsync(selectedPageSize, pageIndex, sorting, filtering, true, false)
  ...
  onPositionsListUpdated(event: {
    positionId: string;
    isLiked: boolean;
  }) {
    if (!event.isLiked) {
      this.dataItems = this.dataItems.filter(
          p => p._id !== event.positionId
      );
      this.totalItems--;
      this.cdr.markForCheck();
    }
